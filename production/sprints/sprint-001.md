...
| S1-03 | ~~**Simplify grid to 2 layers**~~ **CANCELLED** — The passage flag is not redundant. Building top-row (`walkable=true, buildable=false, passage=true`) and mobile-unit-occupied cells (`walkable=true, buildable=false, passage=false`) share the same two-layer state but require different movement behavior. GDD updated to document three-layer model. | Map System GDD | 0h | — | GDD updated to reflect three-layer reality. |
